...
function Bottom() {
  ...
  const renderContributor = (contributor) => (
    <div style={{display: 'flex', alignItems: 'center', width: 'auto', minWidth: 800, height: window.innerHeight * 0.25 -20, justifyContent: 'space-around' }}>
      ...
      <div style={{width: '70%', height: '80%'}}>
        ...
        {contributor.repos.map(repo => (
          <div>
            <h3 style={{color: '#93c2db', margin: 0, padding: 0, fontFamily: "'Inter', sans-serif", fontSize: '1.2em'}}>{repo.split('/')[0]}</h3>
            <h4 style={{color: '#9c4668', margin: 0, padding: 0, fontFamily: "'Inter', sans-serif", fontSize: '1em'}}>{repo.split('/')[1]}</h4>
          </div>
        ))}
        </div>
      </div>
